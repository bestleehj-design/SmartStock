...
class StrategyPipeline:
    ...
    def step2_filter(self, s1_candidates, check_heat=True, check_news=True):
        ...
        heat_map = {}
        if check_heat:
            from theme.hot_rank_collector import get_hot_stocks_cross_check
            try:
        ...
        hot_keywords = set()
        try:
            from theme.hot_rank_collector import get_sustained_hot_boards
        ...
        if check_news and s2_candidates:
            print(f"\n  新闻利好分类（TOP 30）...")
            try:
                from news.news_sentiment_analyzer import NewsCategorizer
    ...
    def step3_score(self, s2_candidates, top_n=10):
        ...
        print(f"\n  基本面分析（对TOP 10）...")
        try:
            from screener.fundamental_analyzer import FundamentalAnalyzer
        ...
        for i, r in enumerate(results[:top_n], 1):
            ...
            # 1. 量价形态
            try:
                from screener.pattern_recognition import PatternRecognizer
            ...
            # 2. 逻辑验证
            try:
                from screener.logic_validator import LogicValidator
                lv = LogicValidator()
            ...
            # 3. 仓位建议
            try:
                from screener.position_sizer import PositionSizer
                ps = PositionSizer()
                # 用策略得分估算胜率
